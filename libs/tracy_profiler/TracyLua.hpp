...
#else
#ifdef TRACY_ON_DEMAND
    auto& st = GetLuaZoneState();
    bool connected = GetProfiler().IsConnected();
    GetLuaZoneState().active = GetProfiler().IsConnected();
    const auto zoneCnt = GetLuaZoneState().counter++;
    if (!connected)
    {
...
static inline int LuaProfileWithSandbox(int line, const char* name, const char* source)
{
#if defined TRACY_HAS_CALLSTACK && defined TRACY_CALLSTACK
    return LuaZoneBeginS(L);
#else
    ...
    const auto srcloc = Profiler::AllocSourceLocation(line, source, name);
    MemWrite(&item->zoneBegin.time, Profiler::GetTime());
    MemWrite(&item->zoneBegin.srcloc, srcloc);
    TracyLfqCommit;
    return 0;
...
static inline int LuaProfileWithSandbox(lua_Debug& dbg)
{
    ...
    TracyLfqPrepare(QueueType::ZoneBeginAllocSrcLoc);
    int line = dbg.currentline >=0 ? dbg.currentline : dbg.linedefined;
    // dbg.name ? dbg.name : dbg.short_src
    const char* name = dbg.name ? dbg.name : "anonymous";
    const char* source = dbg.source ? dbg.source : "C_FUNC";
    if(strcmp(name, "chunkFunc") == 0)
    {
        int a = 1;
        a = 2;
        (void)a;
...
static inline int LuaProfileWithSandbox(lua_Debug& dbg, const char* name, size_t nsz)
{
#ifdef TRACY_ON_DEMAND
    auto st = GetLuaZoneState();
    bool connected = GetProfiler().IsConnected();
