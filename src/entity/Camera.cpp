...
{
    static WindowSystem* windowSystem = nullptr;
    if (windowSystem == nullptr)
    {
        windowSystem = SystemManager::Instance()->GetSystem<WindowSystem>();
    }
    return windowSystem;
}
...
static InputSystem* GetInputSystem()
{
    ...
    {
        inputSystem = SystemManager::Instance()->GetSystem<InputSystem>();
    }
    return inputSystem;
}
...
void Camera::updateProjectionMat4()
{
    ...
    projection = glm::perspective(glm::radians(m_fov), (float)GetWindowSystem()->getWindowWidth() / (float)GetWindowSystem()->getWindowHeight(), m_nearDistance, m_farDistance);
...
bool CameraControl::isTriggered(CameraActions dir) const
{
    ...
    {
        if (glfwGetKey(GetWindowSystem()->getGLFWwindow(), it.first) == it.second)
            return true;
    }
...
void CameraControl::processMouseActions() const
{
    if (!m_mouseEnable)
        return;
    bool mouseMove = GetInputSystem()->IsKeyPressed(GLFW_MOUSE_BUTTON_RIGHT);

    glm::vec2 lastMousePos = GetInputSystem()->getPreMousePos();
    glm::vec2 curMousePos = GetInputSystem()->getCurMousePos();
    ...
    glm::vec2 curScrollValue = GetInputSystem()->getScrollValue();
    if (proxZero(curScrollValue.y) != 0.0f)
    {
        //std::cout << "scroll: " << curScrollValue.x << '\t' << curScrollValue.y << std::endl;
        if (GetInputSystem()->IsKeyPressed(GLFW_KEY_LEFT_SHIFT))
        {
            glm::vec3 camCurPos = m_cam->getCameraPos();
            float width = GetWindowSystem()->getWindowWidth();
            float height = GetWindowSystem()->getWindowHeight();
